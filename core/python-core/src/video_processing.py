...
class VideoProcessing:
    ...
    async def motion_estimation_and_correction(self):
        global_motion_vectors, frame_motion_field_vec, frame_global_motion_vec = await self.motion_estimation.video_processing(self.video, websocket=self.websocket, plot=True)
        ...
        return global_correct_motion_vectors, frame_motion_field_vec, frame_global_motion_vec

    async def post_process_frames(self, global_correct_motion_vectors):
        frames = await self.post_processing.shift_frames(self.video, global_correct_motion_vectors, websocket=self.websocket)
    ...
    async def compare_and_plot(self, frames, global_correct_motion_vectors):
        ...
        fil_crop_gmv, _, _ = await self.motion_estimation.video_processing(video=self.video, frames=filtered_cropped_frames, websocket=self.websocket, update_step_code="me2", compare_message="smoothed")
        del filtered_cropped_frames
        filtered_acc_motion = self.smoothing.get_accumulated_motion_vec(fil_crop_gmv)

        origin_cropped_frames = await self.post_processing.crop_video(video=self.video, max_shift=self.post_processing.max_shift, websocket=self.websocket, update_crop_id="crop_comp_origin", compare_message="origin")
        origin_crop_gmv, _, _ = await self.motion_estimation.video_processing(video=self.video, frames=origin_cropped_frames, websocket=self.websocket, update_step_code="me1", compare_message="origin")
        del origin_cropped_frames
    ...
    async def _process_video_demo(self):
        global_correct_motion_vectors, frame_motion_field_vec, frame_global_motion_vec = await self.motion_estimation_and_correction()
        video_shape= await self.video.get_video_shape()
        global_corrected_vect_frames = utils.plot_global_corrected_motion_vector(global_correct_motion_vectors, video_shape[1], video_shape[0])
    ...
    async def _process_video(self):
        ...
        global_correct_motion_vectors, _, _ = await self.motion_estimation_and_correction()
